...
class TextFileTest: XCTestCase {
    ...
    func testHandleRouteWillReturnResponseWithContentsOfFile1InBody() { 
        let request = buildRequest(method: "GET", route: "/text-file.txt")    
        ...
        XCTAssertEqual(expectedResponse, textFile.handleRoute(request: parsedRequest))
    }

    func testHandleRouteWillReturn405ResponseWhenRequestMethodNotAllowed() { 
        let request = buildRequest(method: "PUT", route: "/text-file.txt")    
        let parsedRequest = parser.parseRequest(request: request)
        let body = readText(request: parsedRequest)
        
        let expectedResponse = buildResponse(statusCode: status405, body: body)
